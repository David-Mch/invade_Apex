/*
	...
	1: (optional) POSITION  - (array) the position (with Z altitude) where the HALO will start from
	...
	[this, [01500,02300,4000], true, true, true] call COB_fnc_HALO
*/
if !(hasInterface) exitWith {};

//Parameters
private ["_unit","_pos","_altitude","_chemLight","_autoOpen","_saveLoadOut"]; 
_unit 		 = [_this, 0, objNull, [objNull]] call BIS_fnc_param;
_pos 	 	 = [_this, 1, (getpos _unit) vectorAdd [0,0,1000], [[]]] call BIS_fnc_param;
...
if (_altitude < 50) exitWith {"Altitude is too low for HALO. Accepted: 50 and greater." call BIS_fnc_error};
...
if (isPlayer _unit) then {
	...
	if ((animationState _unit) isEqualTo "halofreefall_non") then
	{
		_unit spawn {
			while {animationState _this != "para_pilot" && alive _this && !(isTouchingGround _this || (getPos _this select 2) < 1) } do {
	...
	} else
	{
		...
		[_unit] spawn {
			...
			while {animationState _unit != "para_pilot" && alive _unit && !(isTouchingGround _unit || (getPos _unit select 2) < 1) } do {
				playSound "flapping"; //play flapping sound
				sleep 4.2;
			};
		};
...
if (_chemLight) then {
	[_chemLight,_unit] spawn {
		private ["_chemlight","_unit","_light","_light2","_attach"];
		_chemLight = _this select 0;
		_unit	   = _this select 1;

		//_light = "chemlight_red" createVehicle [0,0,0]; //create the chemlight
		_light = "SmokeShellOrange" createVehicle [0,0,0]; //create the chemlight
		hideObjectGlobal _light;
		_light2 = "#lightpoint" createVehicle [0,0,0]; 
		_light2 setLightBrightness 1;
		_light2 setLightAmbient [0.3, 0.1, 0]; 
		_light2 setLightUseFlare false; 
		_light2 setLightFlareSize 1; 
		_light2 setLightFlareMaxDistance 600; 
		_light2 setLightColor [1, 0.5, 0];

		_attach = [_light,_light2];
		
		{
			_x attachTo [_unit,[0,0,0],"spine3"];
		} foreach _attach;

		waitUntil {isTouchingGround _unit || (getPos _unit select 2) < 250 || animationState _unit == "para_pilot"};

		{
			detach _x;
			deleteVehicle _x; //delete the chemlight
		} forEach _attach;
	};
};
...
if (_saveLoadOut && !isNull (unitBackpack _unit) && (backpack _unit) != "b_parachute") then {
	...
	[_unit,_class,_magazines,_weapons,_items,_helmet,_altitude] spawn {
		...
		waitUntil { typeOf (vehicle _unit) isEqualTo "Steerable_Parachute_F" || isTouchingGround _unit || (getPos _unit select 2) < 1 };
		if (typeOf (vehicle _unit) isEqualTo "Steerable_Parachute_F") then
		{
		...
		if !(headgear _unit isEqualTo _helmet) then
		{
...
if (isTouchingGround (vehicle _unit)) then
{
	_unit setPos _pos; //Set the pos and altitude of the HALO jump
};
...
if (isPlayer _unit) then {
	[_unit,_autoOpen] spawn {
		...
		waitUntil {animationState _unit == "para_pilot" || isTouchingGround _unit || (getPos _unit select 2) < 1};
