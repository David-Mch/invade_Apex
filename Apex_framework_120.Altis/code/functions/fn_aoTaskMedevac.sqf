...
if (_state isEqualTo 1) then {
	...
	if (_aoPos inPolygon [
		...
		['ST_MEDEVAC',['Medevac','Evacuar al Soldado herido']] remoteExec ['QS_fnc_showNotification',-2,FALSE];
		[
			'QS_IA_TASK_AO_3',
			TRUE,
			[
				'Un soldado de reconocimiento ha sido herido en el AO y requiere urgencia médica. Llévalo al cuartel general de Medevac en la base (pequeño edificio blanco) para completar la misión. Si muere o se desangra (aproximadamente 60 minutos desde el inicio de la misión), la misión falla. Buena suerte soldados!',
				'Medevac',
				'Medevac'
			],
			[_unit,TRUE],
...
if (_state isEqualTo 2) then {
	...
	if (serverTime > _missionDuration) exitWith {
		...
		['ST_MEDEVAC',['Medevac','¡Medevac falló, el soldado se desangró!']] remoteExec ['QS_fnc_showNotification',-2,FALSE];
	...
	if (!alive _unit) exitWith {
		//comment 'Mission failure';
		['ST_MEDEVAC',['Medevac','Medevac falló, El soldado fue asesinado!']] remoteExec ['QS_fnc_showNotification',-2,FALSE];
		['QS_IA_TASK_AO_3'] call (missionNamespace getVariable 'BIS_fnc_deleteTask');
		_return = [
			_case,
			0,
