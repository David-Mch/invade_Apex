...
if ((!(((attachedObjects player) findIf {((!isNull _x) && { (!(_x isKindOf 'Sign_Sphere10cm_F')) && { !(_x isEqualTo R3F_LOG_joueur_deplace_objet) } })}) isEqualTo -1)) && { (!((toLower _QS_actionText) in [
...
if (_QS_actionName isEqualTo 'GetInPilot') exitWith {
	if ((!(player getUnitTrait 'QS_trait_pilot')) && (!(player getUnitTrait 'QS_trait_armedHeliPilot')) && (!(player getUnitTrait 'QS_trait_fighterPilot'))) then {
	...
	if ("CAS_Heli" in getArray (configFile >> 'CfgVehicles' >> (typeOf _QS_actionTarget) >> 'availableForSupportTypes')) then {
		if (!(player getUnitTrait 'QS_trait_armedHeliPilot')) then {
			_QS_c = TRUE;
			_text = 'Only armed heli pilots can use this aircraft';
			50 cutText [_text,'PLAIN',0.5];
		};
	};
	if (player getUnitTrait 'QS_trait_armedHeliPilot') then {
...
if (_QS_actionName isEqualTo 'GetInGunner') exitWith {
	...
	if (player getUnitTrait 'QS_trait_fighterPilot' || player getUnitTrait 'QS_trait_armedHeliPilot') then {
		_QS_c = TRUE;
	};
	_QS_c;
};
...
if (_QS_actionName isEqualTo 'GetInTurret') exitWith {
	...
	if (['Copilot',(_this # 4),FALSE] call (missionNamespace getVariable 'QS_fnc_inString')) then {
		if (!( (toLower (typeOf (_this # 0))) in ['b_heli_light_01_f','b_heli_light_01_stripped_f','i_heli_light_03_f','i_heli_light_03_unarmed_f','i_heli_light_03_dynamicloadout_f','i_e_heli_light_03_dynamicloadout_f'])) then {
			if ((count allPlayers) > 20) then {
				if ((!(player getUnitTrait 'QS_trait_pilot')) && (!(player getUnitTrait 'QS_trait_fighterPilot')) && (!(player getUnitTrait 'QS_trait_armedHeliPilot'))) then {
					_QS_c = TRUE;
				};
			};
		};
