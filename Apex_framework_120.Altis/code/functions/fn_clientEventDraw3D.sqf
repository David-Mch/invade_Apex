		...
		if (
		...
		) then {
			{
				if ((_player isEqualTo (leader (group _player))) && (_x in (groupSelectedUnits _player))) then {
					...
					drawIcon3D [
						'a3\ui_f\data\igui\cfg\cursors\select_ca.paa',
						[[0,125,255,([1,0.5] select ((getPlayerChannel _x) isEqualTo -1))],[1,0,0,([1,0.5] select ((getPlayerChannel _x) isEqualTo -1))]] select (_x getUnitTrait 'Medic'),
						(if (isNull (objectParent _x)) then {(_x modelToWorldVisual (_x selectionPosition 'Spine3'))} else {((objectParent _x) modelToWorldVisual [0,0,0])}),
						0.5,
						0.5,
						0,
				...
				else
				{
					if (_x getUnitTrait 'Medic') then {
						drawIcon3D [
							'a3\ui_f\data\igui\cfg\cursors\unitHealer_ca.paa',
							[1,1,1,([1,0.5] select ((getPlayerChannel _x) isEqualTo -1))],
							(if (isNull (objectParent _x)) then {(_x modelToWorldVisual (_x selectionPosition 'Spine3'))} else {((objectParent _x) modelToWorldVisual [0,0,0])}),
							0.5,
							0.5,
							0,
				...
				if ((getPlayerChannel _x != -1) && { (_x distance player < 500) }) then {
					drawIcon3D [
						'a3\ui_f\data\igui\rscingameui\rscdisplaychannel\mutevon_ca.paa',
						[1,1,0,([1,0.5] select ((getPlayerChannel _x) isEqualTo -1))],
						(if (isNull (objectParent _x)) then {(_x modelToWorldVisual ((_x selectionPosition 'Neck') vectorAdd [0,0,0.3]))} else {((objectParent _x) modelToWorldVisual [0,0,0])}),
						0.9 + ((sin (time * 360) + 1) * 0.15),
						0.9 + ((sin (time * 360) + 1) * 0.15),
						0,
						[name _x,''] select (cursorObject isEqualTo _x),
						2,
						0.035,
						'PuristaMedium',
						'center',
