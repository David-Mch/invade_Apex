...
_QS_module_swayManager_delay = 0.2;
_QS_module_swayManager_checkDelay = _timeNow + _QS_module_swayManager_delay;
_QS_module_swayManager_secWepSwayCoef = 0.2;
...
_QS_module_swayManager_heavyWeaponCoef_crouch = 0.2;
_QS_module_swayManager_heavyWeaponCoef_stand = 0.2;
_QS_module_swayManager_recoilCoef_crouch = 0.7;
_QS_module_swayManager_recoilCoef_stand = 0.7;
								...
								if ((_QS_player distance2D _QS_module_safezone_pos) > 1000) then {
									...
									if (_QS_currentTimeOnGround > _QS_maxTimeOnGround) then {
										...
										if (_QS_currentTimeOnGround > _QS_warningTimeOnGround) then {
											(missionNamespace getVariable 'QS_managed_hints') pushBack [5,_false,10,-1,'Tú eres un piloto. Por favor, desempeña tu papel o reasigna. Hay más de 30 jugadores en el servidor, reaparece si la evacuación oportuna no está disponible.',[],(serverTime + 20),_true,'Role Restriction',_false];
										};
									};
								} else {
	...
	if (_QS_module_radioChannelManager) then {
		if (_timeNow > _QS_module_radioChannelManager_checkDelay) then {
			if (isNull (uiNamespace getVariable ['QS_client_dialog_menu_roles',displayNull])) then {
				...
				/*/AIRCRAFT/*/
				if ((!(_QS_player getUnitTrait 'QS_trait_pilot')) && (!(_QS_player getUnitTrait 'uavhacker')) && (!(_QS_player getUnitTrait 'QS_trait_HQ')) && (!(_QS_player getUnitTrait 'QS_trait_fighterPilot')) && (!(_QS_player getUnitTrait 'QS_trait_armedHeliPilot')) && (!(_QS_player getUnitTrait 'QS_trait_CAS')) && (!(_QS_player getUnitTrait 'QS_trait_JTAC'))) then {
