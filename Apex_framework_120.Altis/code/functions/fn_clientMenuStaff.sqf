...
if (_type2 isEqualType 0) exitWith {
	...
	if (_type2 isEqualTo 6) then {
		...
		private _text = '';
		{
			...
			if (
				...
				(_x getUnitTrait 'QS_trait_armedHeliPilot') ||
	...
	if (_type2 isEqualTo 7) then {
		...
		if (isNull _cursorTarget) exitWith {
			if (!isStreamFriendlyUIEnabled) then {
				(missionNamespace getVariable 'QS_managed_hints') pushBack [5,TRUE,5,-1,'Reparación fallida, no se seleccionó un objetivo válido',[],-1];
			};
		};
		...
		_text = format ['',profileName,_type2,(mapGridPosition player)];
	...
	if (_type2 isEqualTo 12) then {
		...
		_text = format ['',profileName,(name _cursorTarget),(mapGridPosition _cursorTarget)];
	...
	if (_type2 isEqualTo 14) then {
		_result = ['Base limpia Ejecutar esta función con demasiada frecuencia aumentará la desincronización de la red.','Base cleanup','Execute','Cancel',(findDisplay 46),FALSE,FALSE] call (missionNamespace getVariable 'BIS_fnc_guiMessage');
		if (_result) then {	
			playSound 'ClickSoft';
			[53,[profileName,(getPlayerUID player)]] remoteExec ['QS_fnc_remoteExec',2,FALSE];
			(missionNamespace getVariable 'QS_managed_hints') pushBack [5,TRUE,3,-1,'Limpieza en progreso ...',[],(serverTime + 6)];
			(missionNamespace getVariable 'QS_managed_hints') pushBack [6,TRUE,10,-1,'Base limpiada. Ejecutar esta función con demasiada frecuencia aumentará la desincronización de la red.',[],(serverTime + 20)];
			_text = format ['%1 (staff) ha limpiado el área de desove.',profileName];
			['systemChat',_text] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
		};
	};
