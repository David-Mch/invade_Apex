...
'QS_marker_sideMarker' setMarkerText (format ['%1Objetivo prioritario: Destruir batería antiaérea',(toString [32,32,32])]);
[
	'QS_IA_TASK_SM_0',
	TRUE,
	[
		'El enemigo ha instalado una batería antiaérea cerca de nuestra base. ¡Este es un objetivo prioritario, soldados! ¡Tiene un alcance extremadamente largo! Ve allí y sácalo a toda costa. Mientras está activo, no se recomienda utilizar el transporte aéreo. Este objetivo no está marcado con precisión. Mientras se vuelve a armar (30 segundos), no podrá bloquearse a larga distancia. Se recomienda llevar explosivos pesados.',
		'Destruir Batería antiaérea',
		'Destruir Batería antiaérea'
	],
...
_briefing = parseText "<t align='center' size='2.2'>Objetivo prioritario</t><br/><t size='1.5' color='#b60000'>Destruir Batería antiaérea</t><br/>____________________<br/>OPFOR forces are setting up an anti-air battery to hit you guys damned hard! We've picked up their positions with thermal imaging scans and have marked it on your map.<br/><br/>This is a priority target, boys!";
//['hint',_briefing] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
['NewPriorityTarget',['Destruir Batería antiaérea']] remoteExec ['QS_fnc_showNotification',-2,FALSE];
//comment 'Loop';
...
for '_x' from 0 to 1 step 0 do {
	if (((_aaHulls findIf {(alive _x)}) isEqualTo -1) || {(missionNamespace getVariable ['QS_smSuccess',FALSE])}) exitWith {
		['CompletedPriorityTarget',['Batería antiaérea Neutralizada']] remoteExec ['QS_fnc_showNotification',-2,FALSE];
