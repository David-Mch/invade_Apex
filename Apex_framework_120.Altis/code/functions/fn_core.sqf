...
_QS_remainsCollectorActive = ((_QS_corpseCollector isNotEqualTo 0) || (_QS_wreckCollector isNotEqualTo 0));
...
_QS_darkAccelerationFactor = 100;
_QS_noonAccelerationFactor = 0.5;
_QS_dawnDuskDeccelerationFactor = 100;
	...
	if ((missionNamespace getVariable ['QS_missionConfig_baseLayout',0]) isEqualTo 0) then {
		if (_QS_worldName isEqualTo 'Altis') then {
			_QS_module_recruitableAI_data = [
				...
				[objNull,60,false,{},"B_Patrol_Engineer_F",[14677,16813.4,0.00143814],0,false,0,-1,FALSE],
				[objNull,60,false,{},"B_Patrol_Engineer_F",[14674.9,16812.2,0.00143814],0,false,0,-1,TRUE],
		...
		if (_QS_worldName isEqualTo 'Malden') then {
			_QS_module_recruitableAI_data = [
				...
				[objNull,60,false,{},"B_Patrol_Engineer_F",[8148.38,10151.7,0.00161362],151.663,false,0,-1,TRUE],
				[objNull,60,false,{},"B_Patrol_Engineer_F",[8150.08,10151.7,0.00162315],169.781,false,0,-1,FALSE],
...
if (_QS_system_weekday isNotEqualTo _QS_ladder_weekday) then {
...
if (_QS_module_restart) then {
	...
	_QS_module_restart_script = {
		...
		{
			50 cutText ['Server Name - ' + serverName,'PLAIN',3];
		...
		if (_timeNow > _HVT_checkDelay) then {
			_HVT_isTargeting = _false;
			{
				if ((alive (effectiveCommander _x)) && {((side (group (effectiveCommander _x))) isEqualTo _west)}) then {
					...
					if (_HVT_removeFrom) then {
						if ((_HVT_currentTargets find _QS_v) isNotEqualTo -1) then {
	...
	if (_timeNow > _QS_cleanup_checkDelay) then {
		...
		if (_QS_revealLoudPlayers) then {
			if (_timeNow > _QS_revealLoudPlayers_delay) then {
				...
				if (_knownPlayers isNotEqualTo []) then {
					{
						if ((side _x) in _enemySides) then {
							_grp = _x;
		...
		if (_timeNow > _QS_module_airDefense_checkDelay) then {
			if (_airDefenseAvailable) then {
				if (missionNamespace getVariable ['QS_airbaseDefense',_false]) then {
					...
					['sideChat',[_west,'AirBase'],'Defensa aérea activada!'] remoteExec ['QS_fnc_remoteExecCmd',-2,_false];
				...
				if (_airDefenseOnline) then {
					if (_timeNow > (_airDefenseArray # 1)) then {
						...
						['sideChat',[_west,'AirBase'],'Defensa aérea Desactivada'] remoteExec ['QS_fnc_remoteExecCmd',-2,_false];
					};
				};
				if (_timeNow > (_airDefenseArray # 2)) then {
					_airDefenseAvailable = _true;
					missionNamespace setVariable ['QS_airbaseDefense',_false,_true];
					['sideChat',[_west,'AirBase'],'Defensa aérea disponible'] remoteExec ['QS_fnc_remoteExecCmd',-2,_false];	
				};
			};
			_QS_module_airDefense_checkDelay = _timeNow + _QS_module_airDefense_delay;
		};
